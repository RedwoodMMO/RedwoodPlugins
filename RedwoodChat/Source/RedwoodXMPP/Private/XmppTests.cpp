...
FXmppTest::FXmppTest() :
  bRunBasicPresenceTest(true), bRunPubSubTest(false), bRunChatTest(false) {}

void FXmppTest::Test(
  const FString &UserId, const FString &Password, const FXmppServer &XmppServer
) {
  XmppConnection = FRedwoodXmppModule::Get().CreateConnection(UserId);
  XmppConnection->SetServer(XmppServer);

  OnLoginCompleteHandle =
    XmppConnection->OnLoginComplete().AddRaw(this, &FXmppTest::OnLoginComplete);
...
void FXmppTest::FinishTest() {
  ...
  if (XmppConnection.IsValid()) {
    if (XmppConnection->GetLoginStatus() == EXmppLoginStatus::LoggedIn) {
      OnLogoutCompleteHandle = XmppConnection->OnLogoutComplete().AddRaw(
        this, &FXmppTest::OnLogoutComplete
      );
      ...
      FRedwoodXmppModule::Get().RemoveConnection(XmppConnection.ToSharedRef());
      bDone = true;
    }
  }
  if (bDone) {
...
void FXmppTest::OnLoginComplete(
  const FXmppUserJid &UserJid, bool bWasSuccess, const FString &Error
) {
  UE_LOG(
    LogXmpp,
    Log,
    TEXT("FXmppTest::OnLoginComplete UserJid=%s Success=%s Error=%s"),
    *UserJid.GetFullPath(),
    bWasSuccess ? TEXT("true") : TEXT("false"),
    *Error
  );
...
void FXmppTest::OnLogoutComplete(
  const FXmppUserJid &UserJid, bool bWasSuccess, const FString &Error
) {
  UE_LOG(
    LogXmpp,
    Log,
    TEXT("FXmppTest::OnLogoutComplete UserJid=%s Success=%s Error=%s"),
    *UserJid.GetFullPath(),
    bWasSuccess ? TEXT("true") : TEXT("false"),
    *Error
  );
