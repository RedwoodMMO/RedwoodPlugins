...
IMPLEMENT_MODULE(FRedwoodXmppModule, RedwoodXMPP);

FRedwoodXmppModule *FRedwoodXmppModule::Singleton = nullptr;

void FRedwoodXmppModule::StartupModule() {
  Singleton = this;
...
bool FRedwoodXmppModule::HandleXmppCommand(
  ...
  if (FParse::Command(&Cmd, TEXT("Test"))) {
    ...
    if (UserName.IsEmpty()) {
      ...
      if (!Connection.IsValid()) {
        UE_LOG(
          LogXmpp, Warning, TEXT("No logged in user found for <%s>"), *UserName
        );
      } else {
        Connection->Logout();
    ...
    FString UserName =
      FParse::Token(Cmd, false); // Assumes user is already logged in
    FString OnlineStr =
      FParse::Token(Cmd, false); // "ONLINE, AWAY, DND, OFFLINE, etc"
    FString StatusStr =
      FParse::Token(Cmd, false); // "status message treated as raw string"
    ...
    if (UserName.IsEmpty() || OnlineStr.IsEmpty() || StatusStr.IsEmpty()) {
      UE_LOG(
        ...
        Warning,
        TEXT(
          "Usage: XMPP Presence <username> <ONLINE,AWAY,DND,OFFLINE,XA> <status text>"
        )
      );
    } else {
      TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
      if (!Connection.IsValid()) {
        ...
        if (Connection->Presence().IsValid()) {
          FXmppUserPresence XmppPresence =
            Connection->Presence()->GetPresence();
          ...
          } else if (OnlineStr.Equals(
                       TEXT("OFFLINE"), ESearchCase::IgnoreCase
                     )) {
    ...
    FString UserName =
      FParse::Token(Cmd, false); // Assumes user is already logged in
    FString RosterUser =
      FParse::Token(Cmd, false); // User must be in current roster

    if (UserName.IsEmpty() || RosterUser.IsEmpty()) {
      UE_LOG(
        LogXmpp,
        Warning,
        TEXT("Usage: XMPP PresenceQuery <username> <rosteruser>")
      );
    } else {
      TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
      if (!Connection.IsValid()) {
    ...
    if (UserName.IsEmpty() || Recipient.IsEmpty()) {
      UE_LOG(
        ...
        Warning,
        TEXT(
          "Usage: XMPP Message <username> <recipient> <optional payload string>"
        )
      );
    } else {
      TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
      if (!Connection.IsValid()) {
        ...
        if (Connection->Messages().IsValid()) {
          ...
          Connection->Messages()->SendMessage(
            RecipientId, TEXT("test"), Payload, false
          );
        }
      }
    }
    ...
    if (UserName.IsEmpty() || Recipient.IsEmpty()) {
      UE_LOG(
        LogXmpp, Warning, TEXT("Usage: XMPP Chat <username> <recipient> <body>")
      );
    } else {
      TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
      if (!Connection.IsValid()) {
    ...
    if (FParse::Command(&Cmd, TEXT("Create"))) {
      ...
      FString IsPrivate =
        FParse::Token(Cmd, false); // whether to make room private
      ...
      if (UserName.IsEmpty() || RoomId.IsEmpty() || (IsPrivate == TEXT("1") && Password.IsEmpty()))
        ...
        UE_LOG(
          ...
          Warning,
          TEXT(
            "Usage: XMPP Muc Create <username> <room> <private 1/0> <password>"
          )
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
          UE_LOG(
            LogXmpp,
            Warning,
            TEXT("No logged in user found for <%s>"),
            *UserName
          );
        } else {
          if (Connection->MultiUserChat().IsValid()) {
            ...
            Connection->MultiUserChat()->OnRoomCreated().AddRaw(
              this, &FRedwoodXmppModule::OnXmppRoomCreated
            );
            Connection->MultiUserChat()->OnRoomConfigured().AddRaw(
              this, &FRedwoodXmppModule::OnXmppRoomConfigured
            );
            Connection->MultiUserChat()->CreateRoom(
              RoomId, UserName, RoomConfig
            );
          }
        }
      }
      ...
      FString Nickname = FParse::Token(
        Cmd, false
      ); // nickname to join (must be unique or join will fail)
      FString Password =
        FParse::Token(Cmd, false); // optional password if joining private

      if (UserName.IsEmpty() || RoomId.IsEmpty() || Nickname.IsEmpty()) {
        UE_LOG(
          LogXmpp,
          Warning,
          TEXT("Usage: XMPP Muc Join <username> <room> <nickname> <?password>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
          ...
          if (Connection->MultiUserChat().IsValid()) {
            if (Password.IsEmpty()) {
              ...
              Connection->MultiUserChat()->JoinPrivateRoom(
      ...
      if (UserName.IsEmpty() || RoomId.IsEmpty()) {
        UE_LOG(
          LogXmpp, Warning, TEXT("Usage: XMPP Muc Exit <username> <room>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
      ...
      if (UserName.IsEmpty() || RoomId.IsEmpty() || (IsPrivate == TEXT("1") && Password.IsEmpty()))
        ...
        UE_LOG(
          ...
          Warning,
          TEXT(
            "Usage: XMPP Muc Config <username> <room> <subject> <private 1/0> <password>"
          )
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
      ...
      if (UserName.IsEmpty() || RoomId.IsEmpty()) {
        UE_LOG(
          LogXmpp, Warning, TEXT("Usage: XMPP Muc Refresh <username> <room>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
      ...
      FString RoomId = FParse::Token(Cmd, false); // room to join
      FString Body = FParse::Token(Cmd, false); // payload to send
      FString ExtraInfo = FParse::Token(
        Cmd, false
      ); // optional extra info to send (typically json payload)

      if (UserName.IsEmpty() || RoomId.IsEmpty() || Body.IsEmpty()) {
        UE_LOG(
          LogXmpp,
          Warning,
          TEXT("Usage: XMPP Muc Chat <username> <room> <body>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
    ...
    if (FParse::Command(&Cmd, TEXT("Create"))) {
      ...
      if (UserName.IsEmpty() || NodeId.IsEmpty()) {
        UE_LOG(
          LogXmpp, Warning, TEXT("Usage: XMPP PubSub Create <username> <node>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
      ...
      if (UserName.IsEmpty() || NodeId.IsEmpty()) {
        UE_LOG(
          LogXmpp, Warning, TEXT("Usage: XMPP PubSub Destroy <username> <node>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
      ...
      if (UserName.IsEmpty() || NodeId.IsEmpty()) {
        UE_LOG(
          LogXmpp,
          Warning,
          TEXT("Usage: XMPP PubSub Subscribe <username> <node>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
      ...
      if (UserName.IsEmpty() || NodeId.IsEmpty()) {
        UE_LOG(
          LogXmpp,
          Warning,
          TEXT("Usage: XMPP PubSub Unsubscribe <username> <node>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
      ...
      if (UserName.IsEmpty() || NodeId.IsEmpty() || PayloadStr.IsEmpty()) {
        UE_LOG(
          LogXmpp,
          Warning,
          TEXT("Usage: XMPP PubSub Publish <username> <node> <text>")
        );
      } else {
        TSharedPtr<IXmppConnection> Connection = GetConnection(UserName);
        if (!Connection.IsValid()) {
    ...
    UE_LOG(
      LogXmpp,
      Warning,
      TEXT("Dumping %d XMPP connections: "),
...
bool FRedwoodXmppModule::Exec_Runtime(
...
FRedwoodXmppModule &FRedwoodXmppModule::Get() {
  if (Singleton == nullptr) {
    check(IsInGameThread());
    FModuleManager::LoadModuleChecked<FRedwoodXmppModule>("XMPP");
  }
  check(Singleton != nullptr);
  return *Singleton;
}

bool FRedwoodXmppModule::IsAvailable() {
  return Singleton != nullptr;
}

TSharedRef<IXmppConnection> FRedwoodXmppModule::CreateConnection(
...
TSharedPtr<IXmppConnection> FRedwoodXmppModule::GetConnection(
  const FString &UserId
) const {
...
void FRedwoodXmppModule::RemoveConnection(const FString &UserId) {
...
bool FRedwoodXmppModule::Tick(float DeltaTime) {
  QUICK_SCOPE_CYCLE_COUNTER(STAT_FRedwoodXmppModule_Tick);
  ProcessPendingRemovals();
  return true;
}

void FRedwoodXmppModule::ProcessPendingRemovals() {
  if (PendingRemovals.Num() > 0) {
    PendingRemovals.Empty();
  }
}

void FRedwoodXmppModule::RemoveConnection(
...
void FRedwoodXmppModule::OnXmppRoomCreated(
  ...
  UE_LOG(
    LogXmpp,
    Log,
    TEXT("FRedwoodXmppModule::OnXmppRoomCreated - entered - user(%s) room(%s)"),
    *Connection->GetUserJid().Id,
    *RoomId
  );
}

void FRedwoodXmppModule::OnXmppRoomConfigured(
  const TSharedRef<IXmppConnection> &Connection,
  bool bSuccess,
  const FXmppRoomId &RoomId,
  const FString &Error
  ...
  UE_LOG(
    ...
    Log,
    TEXT(
      "FRedwoodXmppModule::OnXmppRoomConfigured - entered - user(%s) room(%s)"
    ),
    *Connection->GetUserJid().Id,
    *RoomId
  );
